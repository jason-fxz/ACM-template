...
// 求 n! mod pk^tk, 返回值 U{ 不包含 pk 的值 ,pk 出现的次数 }
U fac(int k, LL n) { 
    ...
    return Pw(p[k], a.z - b.z - c.z, P[k]) * a.x % P[k] * inv(b.x, P[k]) % P[k] * inv(c.x,
           P[k]) % P[k];
} LL CRT() { // CRT 合并答案
    LL d, w, y, x, ans = 0;
    fr(i, 1, l)w = M / P[i], exgcd(w, P[i], x, y), ans = (ans + w * x % M * a[i]) % M;
    return (ans + M) % M;
    ...
    for (i = 2; i * i <= jj; i++)if (jj % i == 0) for (p[++l] = i, P[l] = 1; jj % i == 0;
                P[l] *= p[l])jj /= i;
