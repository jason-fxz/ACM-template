...
namespace KDT {
    ...
    int build(int l, int r) { // 对 g[1...t] 进行建树 ，对应点都是 g[x]。方差建树
        if (l > r) return 0;
        int mid = (l + r) >> 1;
...
int main() {
    ...
    for (int op;;) {
        ...
        switch (op) {
        case 1:
            ++cur; read(p[cur][0]); read(p[cur][1]); read(val[cur]);
            insert(root, cur);
            break;
        case 2:
            read(Lt[0]); read(Lt[1]); read(Rt[0]); read(Rt[1]);
            printf("%d\n", query(root));
            break;
